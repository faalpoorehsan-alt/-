...
export function parseMilkMessageLocally(text: string, contactName: string = "مشتری"): LocalParsedResult {
  ...
  // 2. Check for Incremental (addition) patterns
  // Examples: "۳۰ تا دیگه بذار روش", "۲۰ کیلو اضافه کن", "۱۰ تا بیشتر", "علاوه بر قبلی"
  const incrementKeywords = [
    "دیگه", "اضافه", "بذار روش", "بزار روش", "علاوه", "اضاف", "روش", "بیشتر", "دیگر"
  ];
  const hasIncrementKeyword = incrementKeywords.some(keyword => normalizedText.includes(keyword));

  // 3. Check for general milk requests keywords to confirm it's related to milk
  const milkKeywords = [
  ...
  // 5. Determine if it is a valid milk order request
  ...
  if (quantity !== null && (hasMilkKeyword || normalizedText.length < 50)) {
    isMilkRequest = true;
    const isIncremental = hasIncrementKeyword;
    if (isIncremental) {
      explanation = `درخواست اضافه کردن ${quantity} کیلوگرم شیر دیگر به سفارش قبلی`;
    } else {
